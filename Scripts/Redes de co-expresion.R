###############################################################
## Tema: Redes de co-expresión                               ##
## Autor: Olga Andrea Hernandez Miranda, Miranda H           ##
## Fecha: 05/09/2024                                         ##
## Nota: https://www.youtube.com/watch?v=Q-1EpHE_4Pc&t=683s  ##
###############################################################
...
diff.expr <- fread("Redh.csv", header = T)
diff.expr
idGene<-fread("IDH.csv", header = T)
...
#Data frame con datos de etapas y genes
...
#Se seleccionó la matriz que contiene todos los datos para sacar correlación

expr<-allexpr
#expr<-allexpr[grep("Pre-pol",row.names(allexpr)),]
gene.correlation <- cor(expr)
gene.correlation

#Crar CSV para quitar NA manualmente 
#write.csv(gene.correlation, "correlation_h.csv", row.names = T, col.names = T)

#Cargar matriz sin NA
data <- read.table("correlation_h.csv", sep = ",", header = T)
row.names(data) <- data[,1]
mat_data <- data.matrix(data[,1:ncol(data)]) 
gene.correlation2 <- mat_data[,-1]
gene.correlation2

#Se evalua una serie de valores de correlaci?n para establecer el punto adecuado de correlaci?n que recupera 
#una mejor construcci?n de red. En este ejemplo vamos a cribar un espectro de valores entre 0.5 hasta 0.99
...
#write.graph(gene.coexpression.network,file="gene_coexpression_network2_TFF.gml",format="gml")
...
ggplot(df_degrees, aes(x = Degree)) +
  geom_histogram(aes(y = ..count..), binwidth = 1, fill = "#680280", alpha = 0.8) + # Ajuste de transparencia aquí
  labs(x = "Grado de nodo (K)",
       y = "Número de nodos con K enlaces [P(K)]",
       title = NULL) +
  theme_classic() +
  theme(axis.text = element_text(size = 20),  # Ajustar el tamaño de la fuente de los ejes
        axis.title = element_text(size = 24), # Ajustar el tamaño de la fuente de los títulos de los ejes
        plot.title = element_text(size = 24)) # Ajustar el tamaño de la fuente del título del gráfico

#write.table(network.degrees,file = "grados de nodo_TFF")
...
ggplot(data, aes(x = Log10Degree, y = Log10Frequency)) +
  ...
  geom_smooth(method = "lm", colour = "#680280", fill = "#680280", alpha = 0.2, se = TRUE) +  # Añade la línea de regresión con sombra morada y transparencia
  xlab("log10 grado de nodo (K)") +
...
#write.table(hub.score.attributes,file = "hub_scoreEt_TFF")
...
colors <- c("PAM" = "royalblue", "HCLUST" = "#680280")
